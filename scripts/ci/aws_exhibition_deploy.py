"""Dispatch and follow the exact wgModelHub SHA in the Ohio exhibition pipeline.

路演没有 AgentStream；Test/Build 成功后自动放行 ApproveApplicationRelease。DetectChanges 关闭，只接受 GitHub 精确 SHA。
"""
...
def maybe_approve(execution_id, sha):
    ...
    if test_status != "Succeeded" or build_status != "Succeeded":
        return False
    summary = "GitHub exhibition approved run {}, sha {}".format(
...
def write_evidence(payload):
    path = Path("artifacts/aws-exhibition-deployment.json")
...
def main():
    ...
    if os.environ.get("GITHUB_REF") != "refs/heads/dev" or not re.fullmatch(r"[0-9a-f]{40}", sha):
        raise RuntimeError("AWS exhibition deployment requires the actual dev ref and full commit SHA")
    ...
    with open(os.environ["GITHUB_STEP_SUMMARY"], "a", encoding="utf-8") as summary:
        summary.write(
            "### AWS exhibition deployment succeeded\n\n"
            f"Commit: `{sha}`\n\n"
            "Service: `tomirro/modelhub-service`\n\n"
