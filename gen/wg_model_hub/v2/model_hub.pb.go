...
type GenerationTaskState int32

const (
	GenerationTaskState_GENERATION_TASK_STATE_UNSPECIFIED GenerationTaskState = 0
	GenerationTaskState_GENERATION_TASK_STATE_PENDING     GenerationTaskState = 1
	GenerationTaskState_GENERATION_TASK_STATE_RUNNING     GenerationTaskState = 2
	GenerationTaskState_GENERATION_TASK_STATE_SUCCEEDED   GenerationTaskState = 3
	GenerationTaskState_GENERATION_TASK_STATE_FAILED      GenerationTaskState = 4
)

// Enum value maps for GenerationTaskState.
var (
	GenerationTaskState_name = map[int32]string{
		0: "GENERATION_TASK_STATE_UNSPECIFIED",
		1: "GENERATION_TASK_STATE_PENDING",
		2: "GENERATION_TASK_STATE_RUNNING",
		3: "GENERATION_TASK_STATE_SUCCEEDED",
		4: "GENERATION_TASK_STATE_FAILED",
	}
	GenerationTaskState_value = map[string]int32{
...
func (x GenerationTaskState) Enum() *GenerationTaskState {
	p := new(GenerationTaskState)
	*p = x
	return p
}

func (x GenerationTaskState) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}
...
type TextOutput struct {
	...
	// thinking_budget 是调用方给出的思考 token 上限。
	// 只认数字的模型在没有档位、或档位和数字同时出现时使用它；认档位的模型有档位时丢掉它。
	// 两个字段可以同时填写，ModelHub 不会因此失败，也不会把档位和数字一起发给上游。
	// optional 保留显式 0。
	...
	// thinking_level 与生图共用枚举。
	// 认档位的模型有档位就发档位并丢掉预算；只认数字的模型有预算就用预算，没预算才把档位换成数字。
	ThinkingLevel ThinkingLevel `protobuf:"varint,7,opt,name=thinking_level,json=thinkingLevel,proto3,enum=wg_model_hub.v2.ThinkingLevel" json:"thinking_level,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *TextOutput) GetThinkingLevel() ThinkingLevel {
	if x != nil {
...
type SubmitGenerationRequest struct {
	...
	Request       *GenerateRequest       `protobuf:"bytes,2,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *SubmitGenerationRequest) GetRequest() *GenerateRequest {
	if x != nil {
		return x.Request
	}
	return nil
...
type GenerationTask struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TaskId        string                 `protobuf:"bytes,1,opt,name=task_id,json=taskId,proto3" json:"task_id,omitempty"`
	State         GenerationTaskState    `protobuf:"varint,2,opt,name=state,proto3,enum=wg_model_hub.v2.GenerationTaskState" json:"state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GenerationTask) GetState() GenerationTaskState {
	if x != nil {
		return x.State
	}
	return GenerationTaskState_GENERATION_TASK_STATE_UNSPECIFIED
}

type GenerationTaskStatus struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	State GenerationTaskState    `protobuf:"varint,1,opt,name=state,proto3,enum=wg_model_hub.v2.GenerationTaskState" json:"state,omitempty"`
	// PENDING/RUNNING 时提示调用方下次 Get 的间隔；终态可为 0。
	PollAfterMs int32 `protobuf:"varint,2,opt,name=poll_after_ms,json=pollAfterMs,proto3" json:"poll_after_ms,omitempty"`
...
func (x *GenerationTaskStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *GenerationTaskStatus) GetPollAfterMs() int32 {
	if x != nil {
		return x.PollAfterMs
	}
	return 0
}

func (x *GenerationTaskStatus) GetError() *status.Status {
	if x != nil {
		return x.Error
	}
	return nil
...
type GenerationTaskEvent struct {
	...
	//
	//	*GenerationTaskEvent_Status
	//	*GenerationTaskEvent_Output
	Event         isGenerationTaskEvent_Event `protobuf_oneof:"event"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GenerationTaskEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *GenerationTaskEvent) GetStatus() *GenerationTaskStatus {
	if x != nil {
		if x, ok := x.Event.(*GenerationTaskEvent_Status); ok {
			return x.Status
		}
	}
	return nil
...
func (x *GenerationTaskEvent) GetOutput() *GenerateEvent {
	if x != nil {
		if x, ok := x.Event.(*GenerationTaskEvent_Output); ok {
			return x.Output
		}
	}
	return nil
...
type isGenerationTaskEvent_Event interface {
	isGenerationTaskEvent_Event()
}

type GenerationTaskEvent_Status struct {
	Status *GenerationTaskStatus `protobuf:"bytes,1,opt,name=status,proto3,oneof"`
}

type GenerationTaskEvent_Output struct {
	Output *GenerateEvent `protobuf:"bytes,2,opt,name=output,proto3,oneof"`
}

func (*GenerationTaskEvent_Status) isGenerationTaskEvent_Event() {}
...
const file_proto_wg_model_hub_v2_model_hub_proto_rawDesc = "" +
	...
	"\x0eexpire_at_unix\x18\x02 \x01(\x03R\fexpireAtUnix\"\xbc\x03\n" +
	"\n" +
	"TextOutput\x12/\n" +
	...
	"\x0fthinking_budget\x18\x06 \x01(\x05H\x03R\x0ethinkingBudget\x88\x01\x01\x12E\n" +
	"\x0ethinking_level\x18\a \x01(\x0e2\x1e.wg_model_hub.v2.ThinkingLevelR\rthinkingLevelB\x14\n" +
	...
	"\x15generation_elapsed_ms\x18\b \x01(\x03R\x13generationElapsedMs\"t\n" +
	"\x17SubmitGenerationRequest\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12:\n" +
	"\arequest\x18\x02 \x01(\v2 .wg_model_hub.v2.GenerateRequestR\arequest\"/\n" +
	"\x14GetGenerationRequest\x12\x17\n" +
	"\atask_id\x18\x01 \x01(\tR\x06taskId\"e\n" +
	"\x0eGenerationTask\x12\x17\n" +
	"\atask_id\x18\x01 \x01(\tR\x06taskId\x12:\n" +
	"\x05state\x18\x02 \x01(\x0e2$.wg_model_hub.v2.GenerationTaskStateR\x05state\"\xa0\x01\n" +
	"\x14GenerationTaskStatus\x12:\n" +
	"\x05state\x18\x01 \x01(\x0e2$.wg_model_hub.v2.GenerationTaskStateR\x05state\x12\"\n" +
	"\rpoll_after_ms\x18\x02 \x01(\x05R\vpollAfterMs\x12(\n" +
	"\x05error\x18\x03 \x01(\v2\x12.google.rpc.StatusR\x05error\"\x99\x01\n" +
	...
	"\x13THINKING_LEVEL_HIGH\x10\x04*\xc9\x01\n" +
	"\x13GenerationTaskState\x12%\n" +
	"!GENERATION_TASK_STATE_UNSPECIFIED\x10\x00\x12!\n" +
...
var file_proto_wg_model_hub_v2_model_hub_proto_depIdxs = []int32{
	...
	8,  // 49: wg_model_hub.v2.ModelHubService.ListModels:input_type -> wg_model_hub.v2.ListModelsRequest
	33, // 50: wg_model_hub.v2.ModelHubService.Generate:output_type -> wg_model_hub.v2.GenerateEvent
	22, // 51: wg_model_hub.v2.ModelHubService.CreateCachedContent:output_type -> wg_model_hub.v2.CreateCachedContentResponse
	36, // 52: wg_model_hub.v2.ModelHubService.SubmitGeneration:output_type -> wg_model_hub.v2.GenerationTask
	38, // 53: wg_model_hub.v2.ModelHubService.GetGeneration:output_type -> wg_model_hub.v2.GenerationTaskEvent
	40, // 54: wg_model_hub.v2.ModelHubService.SynthesizeSpeech:output_type -> wg_model_hub.v2.SynthesizeSpeechResponse
	10, // 55: wg_model_hub.v2.ModelHubService.ListModels:output_type -> wg_model_hub.v2.ListModelsResponse
